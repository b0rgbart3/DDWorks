...
//	$_SESSION['path'] = "../info/";
	...
	//echo "<BR>GET: " . $_GET['order']."<br>";
	$url = $_SERVER['REQUEST_URI'];
	//var_dump($_GET);
	// $parts = parse_url($url);
	// parse_str($parts['cat'], $query);
	// echo $query['cat'];
	...
	// //	echo "<BR>SETTING ORDER TO: ".$order."<BR>";

	//   }  else {
	// 	//  echo "<BR>NO ORDER SET<BR>";
	//   }


	// function segment_work($cat) {
	...
	// if (isset($sort)) {
	// 	echo "<script>$( function() { window.artquery.presetSort('".$sort."'); });</script>";
	// }
	// switch($tab) {
	// 	case 'collection':
	// 	echo "<script>$( function() {displaySection('".$cat."');});</script>";
	// 	break;
	// 	case 'list':
	// 	echo "<script>$( function() {displayTab('list');});</script>";
	// 	break;
	// 	case 'shows':
	// 	echo "<script>$( function() {displayTab('shows');});</script>";
	// 	break;
	// 	case 'archive':
	// 	echo "<script>$( function() { displayTab('archive'); });</script>";
	// 	break;
...
?>
<div class='tab tab_selected' data-tab='collection' id='collectionTab'>Collection</div>
<div class='tab' data-tab='archive' id='archiveTab'>Archive</div>
<div class='tab' data-tab='list' id='listTab'>List</div>
...
if (sizeof($works) < 1) {
	...
	echo "<ul class='filter'><li id='created_filter'>Sort By Date</li>
	...
	//	echo 'here';
				...
				// echo "<div class='section adminContent' cellpadding='0' cellspacing='0' >";
				// echo "<p class='countData'>".$count." pieces:";
				// if ($missing > 0) { echo " ".$missing." missing images.</p>"; } 
				// else { echo "."; }
				// foreach($work as $workIndex => $awork) {
				// 	if (!$awork->archived) {
